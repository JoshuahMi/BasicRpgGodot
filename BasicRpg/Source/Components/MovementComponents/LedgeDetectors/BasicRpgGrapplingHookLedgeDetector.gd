...
func get_base_point_refactored() -> BasicRpgHitResult:
	...
	if debug:
		...
		if base_point_0.valid:
			DebugShapes.place_the_red_sphere(base_point_0.position)
		if base_point_1.valid:
			DebugShapes.place_the_purple_sphere(base_point_1.position)
...
func get_ledge_from_base_point_refactored(base_point: BasicRpgHitResult) -> BasicRpgLedge:
	...
	if surface_scan.is_flat_surface:
		for i in 8:
			surface_scan = scan_surface(surface_scan.results[surface_scan.results.size() - 1], vertical_row_y_tolerance * (i + 1))
